...
session_start();
$username = $registration = $email = $mobile = $password = $confirm_password = "";
$username_err = $registration_err = $email_err= $mobile_err = $password_err = $confirm_password_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["username"]))){
        ...
        $sql = "SELECT registration FROM hospital_base WHERE username = ?";
    ...
    if(empty(trim($_POST["registration"]))){
      $registration_err = "Please enter a registration number.";
  } elseif(!preg_match('/^[a-zA-Z0-9]+$/', trim($_POST["registration"]))){
      $registration_err = "registration can only contain letters, numbers";
  } else{
    ...
    if(empty($username_err) && empty($registration_err) && empty($email_err) && empty($mobile_err) && empty($password_err) && empty($confirm_password_err)){
        
        // Prepare an insert statement
        $sql = "INSERT INTO hospital_base (username,registration,email,mobile, password) VALUES (?, ?, ? ,? ,?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssss", $param_username,$param_registration,$param_email, $param_mobile, $param_password);
